...
data_id = 2#int(input("Enter the number of datasets : \n"))

Cos = ["7.5e-6","1e-5", "2.5e-5"]
colors = {"7.5e-6":'b', "1e-5":'k',"2.5e-5":'r'}
typeline = {"7.5e-6":'--', "1e-5":'-', "2.5e-5":'--'}
...
for n in range(data_id):
	...
	ax1.plot(t_array, smooth(ytip_array,19), typeline[Cos[n]], color = colors[Cos[n]], linewidth=0.8, label='dt = '+Cos[n])
	ax1.legend(loc="lower center", fontsize = 12, ncol=len(Cos))



plt.savefig("Tip deflection History.png", dpi=600)
